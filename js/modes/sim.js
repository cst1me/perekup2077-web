...
function renderGar() {
  ...
  gc.innerHTML = G.gar.map(function(c) {
    ...
    return '<div class="scar" onclick="openCarModal(\'' + c.id + '\')">' +
      '<div class="scar-emoji">' + c.e + '</div>' +
      '<div class="scar-name">' + c.n + '</div>' +
      (dmgCount ? '<div class="damage-indicator">🩹 ' + dmgCount + ' повр.</div>' : '<div class="damage-indicator ok">✅ Отлично</div>') +
      '<div class="scar-stats">' +
        '<div class="sstat"><div class="sstat-lbl">Пробег</div><div class="sstat-val">' + fmt(c.mi) + ' км</div></div>' +
        '<div class="sstat"><div class="sstat-lbl">Сост.</div><div class="sstat-val">' + c.cond + '%</div></div>' +
      '</div>' +
...
function updateSrvSelect() {
  ...
  sel.innerHTML = '<option value="">— Выберите машину —</option>' + 
    G.gar.map(function(c) { 
      return '<option value="' + c.id + '">' + c.e + ' ' + c.n + ' (' + c.cond + '%)</option>'; 
    }).join('');
  
  // Восстанавливаем выбор если машина ещё есть
  if (selectedSrvCarId) {
...
function buyG(id) {
  // Shadow Save BEFORE a critical mutation.
  // If something goes wrong mid-flight, player can roll back.
