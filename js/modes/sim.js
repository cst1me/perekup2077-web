...
import { fmt, rnd, pick, clamp, toast, vibrate, getTodayKey } from '../core/utils.js';
import { cars, comments, names, avas, srvs, DAILY_EVENTS, TAXI_DAILY_LIMIT } from '../core/data.js';
import { G, GS, saveGlobalStats, loadTaxiDaily, saveTaxiDaily, taxiUseOne, hiddenLevel } from '../core/state.js';
...
export export function startSim(){
  enterFullscreen();
  show('sim-screen');
...
export export function renderSim(){ updG(); renderMkt(); renderGar(); renderSrv(); renderTaxi(); renderSk(); }

export function updG(){
...
export function renderGar(){
  ...
  gc.innerHTML = G.gar.map(c=>{
    ...
    return `<div class="scar" onclick="openModal('${c.id}')">
      <div class="scar-emoji">${c.e}</div>
      <div class="scar-name">${c.n}</div>
...
export function renderTaxi(){
  const tc = document.getElementById('taxi-card');
  if(!G.gar.length){ tc.innerHTML='<div class="empty"><div class="empty-icon">🅿️</div>Нужна тачка</div>'; return; }
  if(!G.taxi) genTaxi();
  const o = G.taxi;
  const left = taxiLeft();
  tc.innerHTML = `
...
function applySrv(sid){
  ...
  if(s.e.m){ c.mi = Math.max(1000,c.mi+s.e.m); addXP('хитрость',50); }
  if(s.e.r) c.pr = true;
  c.vm = c.vm||1;
...
function skipTaxi(){ genTaxi(); renderTaxi(); }
...
export function refreshMarket(){
  ...
  if(Math.random()<0.85){
    ...
    for(const e of DAILY_EVENTS){
      r -= e.w;
      if(r<=0){ G.lastEvent={t:e.t,d:e.d}; e.run(); break; }
    }
  } else {
    G.lastEvent = null;
  }
  ...
  if(upkeep>0){ G.m-=upkeep; toast(`📅День ${G.day} • -${fmt(upkeep)}₽`); }
  else toast(`📅День ${G.day}`);
...
function addXP(sk,am){
  ...
  if(!s) return;
  s.x += am;
  if(s.x>=s.l*100){ s.x-=s.l*100; s.l++; toast(`🎯${sk}→${s.l}`); }
  renderSk();
}
...
function renderModal(){
  ...
  let dmgHtml = '';
  if(c.damages?.length){
    dmgHtml = `<div class="modal-sec"><div class="modal-sec-title">🩹 Повреждения</div><div class="dmg-list">${
      c.damages.map(d=>`<div class="dmg"><div class="dmg-row"><div class="dmg-part">${d.part}</div><div class="dmg-sev">${d.severity}%</div></div><div class="dmg-bar"><div class="dmg-fill" style="width:${d.severity}%"></div></div></div>`).join('')
    }</div></div>`;
  }
...
document.getElementById('srv-select')?.addEventListener('change',function(){
  selectedSrvCarId = this.value;
